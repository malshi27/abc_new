        ...
        <>
            ...
            <header id="header" className={`fixed-top ${!show && "stickyHeader"}`}>
                <div className="container d-flex align-items-center">
                    <Link style={{ width: '200px' }} to={'/'} className="logo me-auto ">
                    <h5 className="img-fluid">
  <span style={{ color: 'red' }}>ABC</span> Restaurant
</h5>
                    </Link>
                    <HeaderNav isLoggedIn={isLoggedIn} data={data}
                        avatar={avatar} content={content} open={open} setOpen={setOpen} />
                    <Link to={'/track-appointment'} className="appointment-btn scrollto"><span className="d-none d-md-inline">Track</span> reservation</Link>
                </div>
            </header>
        </>
    )
